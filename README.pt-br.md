...
- [Referência de Comandos](#%EF%B8%8F-referência-de-comandos)
...
| **Workflow**  | `2. COMMIT`     | Escaneia modificações, prepara (stage) e comita as alterações com segurança.      |
...
| **Tático**   | `8. AUDIT`      | Aciona o `git diff`para inspecionar o código linha por linha antes do commit.       |
| **Tático**   | `9. RADAR`      | Executa um `git fetch --all`passivo para verificar atualizações na nuvem.          |
| **Tático**   | `10. REVERT`    | **Botão de Pânico:**Realiza um `soft reset`, mantendo os arquivos na staging area. |
| **Tático**   | `11. DISCARD`   | **Protocolo de Destruição:**Descarta todas as alterações locais não comitadas.    |
...
| **Sistema**   | `17. SHIELD`    | **Shield Ops:**Injeta um pre-commit hook para bloquear vazamento de segredos.          |

---

## 📚 Casos de Uso DevSecOps

### 1. O Protocolo de Resiliência Multi-Cloud (Omnicast)

Depender de um único ponto de falha para o código-fonte é um risco de segurança.
...
### 2. Prevenção Ativa de Vazamentos (Shield Ops)

Garanta que nenhum segredo chegue ao seu repositório remoto.
...
### 3. Mitigação de Exposição Acidental de Credenciais
...
3. O commit é apagado do histórico, mas seus arquivos permanecem preparados (staged). Faça as correções necessárias e use a opção `2` ( **COMMIT** ) para criar um registro limpo e seguro.

---

## 🛡️ Política de Segurança

Como um projeto mantido pela  **Chavatte Security** , levamos a integridade de nossas ferramentas a sério. Se você descobrir uma vulnerabilidade ou um possível vetor de ataque nos scripts da CLI (como falhas de injeção de comandos), por favor, **não** abra uma issue pública.

Em vez disso, entre em contato diretamente com o mantenedor para organizar uma divulgação (disclosure) segura.
